...
// This one happens too often to save metrics each time.
#define inc_metric_running_time() \
  get_metric_running_time()++;

// Called when normal incremental saving isn't working.
#define inc_metric_meta() \
  get_metric_meta()++; save_eeprom_fully();

#define inc_metric_sw_hours() \
  get_metric_sw_hours()++; save_metrics();

#define inc_metric_sw_mins() \
  get_metric_sw_mins()++; save_metrics();

#define inc_metric_sw_secs() \
  get_metric_sw_secs()++; save_metrics();
